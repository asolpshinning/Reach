...
import * as backendCtc from '../reachBackend/raffle.main'
...
const User = API({
    ...
    getTicket : Fun([UInt], Bool, ["Buy A Ticket"], `To win the Raffle, you can buy one ticket per time, and later buy more if you wish`),
    revealWinner : Fun([UInt, UInt], Bool, ['salt', 'winningTicket'], `Let us us see who the winner is`),
    claimRewards : Fun([], Bool, [], `This is where to claim raffle rewards, if you are the winner`),
});
//
  ...
  const deploySmartContract = async() => {
        ...
        await Promise.all([
            backendCtc.Creator(ctcCreator, {
                ...
                showSalt: (msg) => {
                    console.log(msg);
                    alert(`Here is the salt (save for your record): ${msg}`);
                },
                showWinningTicket: (msg) => {
                    console.log(msg);
                    alert(`Here is the winning ticket (save for your record): ${msg}`);
                },
                getParams: (msg) => {
                    ...
                    return {NFT: `101882925`, ticketPrice: stdlib.parseCurrency(1) }
...
//
  return (
    <div className={styles.container}>
      <h1>REACH BEAR CHALLENGE: NFT Raffle</h1>
